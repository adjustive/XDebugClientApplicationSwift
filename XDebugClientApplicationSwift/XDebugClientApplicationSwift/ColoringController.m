...
- (void) setTextView: (NSTextView*)textView_i
{
    textview = textView_i;
    [self setup];
}
...
- (void)setup
{
    keywordColor = [NSColor blueColor];
    stringColor = [NSColor colorWithCalibratedRed:0.796 green:0.325 blue:0.098 alpha:1.00]; // orange
    variableColor = [NSColor colorWithCalibratedRed:0.114 green:0.408 blue:0.196 alpha:1.00]; // green
    commentColor = [NSColor brownColor];
    ...
    file = [NSString stringWithContentsOfFile:[[NSBundle mainBundle] pathForResource:@"PHP Keywords" ofType:@"txt"] encoding:NSUTF8StringEncoding error:nil];
...
- (void)startCompletionTimer {
    ...
    [self stopCompletionTimer];
    completionTimer = [NSTimer scheduledTimerWithTimeInterval:COMPLETION_DELAY target:self selector:@selector(doCompletion:) userInfo:nil repeats:NO];
}
